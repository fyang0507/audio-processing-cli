...
- Make the plan answerable in two steps, and generate its sample output rather
  ...
  stacks against ten requestable capabilities is roughly 4,096 combinations, so a
...
- Separate floors from choices. Six floors, none of them optional: punctuated and
  sentence-segmented text (so FireRed always runs FireRedPunc); punctuation
  attached to its word rather than emitted as a parallel stream, because cue
  splitting reads the mark off the token; the canonical source timeline; no
  synthesized bounds; abstention survival; and normalization at the adapter
  boundary. The punctuation floor is the one with a named live risk — FireRedPunc
  emits marks with their own bounds — so its adapter reattaches the mark and drops
  the mark's bounds, leaving the word's own bounds untouched.
...
word timing; FireRed declares native word time and confidence, native speech
bounds, and a region language label and no speakers. The adapter boundary and the
rule that container bounds never become timestamps are under load immediately.
...
options are parked as a follow-up issue. Timing quality is unvalidated: boundary
MAE/P95 is unmeasured for both FireRed's native times and the aligner.
...
text, language, external and native bounds, word timing, confidence, anonymous
speaker, capture role, and abstentions. A capability no backend produced must
stay missing rather than be synthesized from an unrelated field. Backend
...
- Add a punctuation-reattachment test per stack: every mark arrives inside a word
  token, no parallel punctuation stream survives the adapter, and no word's bounds
  grew to cover a mark. A floor with a named live risk and no test is only prose.
- Assert that a plan's `sample_output` key set equals a real run's key set, and
  ...
  single source of truth, including its distinction between the two exit-2 codes.
