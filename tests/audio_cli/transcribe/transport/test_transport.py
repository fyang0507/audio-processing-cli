...
def test_environment_stage_scripts_do_not_import_core_package() -> None:
    stages = Path(__file__).parents[4] / "src/audio_cli/transcribe/stages"
...
def test_stage_sources_pin_the_measured_private_api_and_aligner_language_rule() -> None:
    ...
    inference = (
        Path(__file__).parents[4] / "model_tests/benchmark/turn_attributed_mlx_asr/inference.py"
...
def test_turn_attributed_runner_provenance_hashes_every_source(monkeypatch) -> None:
    benchmark = Path(__file__).parents[4] / "model_tests/benchmark"
