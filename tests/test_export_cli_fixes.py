...
def test_output_exists_fix_runs_for_option_like_paths(tmp_path: Path, monkeypatch, capsys) -> None:
...
def test_export_cli_timing_refusal_has_a_runnable_transcribe_fix(tmp_path: Path, capsys) -> None:
    source = tmp_path / "source.wav"
    source.write_bytes(b"source")
    ...
    assert error["found"] == []
    assert error["fix"].startswith(f"audio transcribe run --input {source} --stack qwen-0.6b")
...
def test_export_cli_timing_fix_chooses_an_unused_result_path(tmp_path: Path, capsys) -> None:
    source = tmp_path / "source.wav"
    source.write_bytes(b"source")
    transcript = _write_result(
        tmp_path / "meeting.transcript.json",
    ...
    (tmp_path / "meeting.timed.2.partial.json").write_text("occupied", encoding="utf-8")
