...
KEY_MAP = {"start": "start_time", "end": "end_time", "speaker_id": "speaker_id", "text": "text"}
...
def normalized_hash(segments: list[dict]) -> str:
    """The recorded recipe (run_vibevoice.py:321-324,474-475), unchanged."""
...
def rename_keys(segments: list[dict]) -> list[dict]:
    return [
        {KEY_MAP.get(key, key): value for key, value in segment.items()} for segment in segments
    ]
...
def compare(mine: list[dict], theirs: list[dict], duration: float) -> dict:
    ...
    paired = list(zip(mine, theirs, strict=False))
    ...
    starts = [
        abs(float(a["start_time"]) - float(b["start_time"]))
        for a, b in paired
        if a.get("start_time") is not None and b.get("start_time") is not None
    ]
    ends = [
        abs(float(a["end_time"]) - float(b["end_time"]))
        for a, b in paired
    ...
    result["segment_bound_deltas"] = {
        "start": summarize(starts),
        "end": summarize(ends),
        "note": (
            "Pairwise by index, valid only while segment counts match; neither side is "
            "ground truth."
        ),
    }

    result["mlx_speaker_labels"] = sorted(
        {
    ...
    result["torch_speaker_labels"] = sorted(
        {
            str(segment.get("speaker_id"))
    ...
    result["mlx_non_label_speaker_segments"] = [
        index for index, segment in enumerate(mine) if str(segment.get("speaker_id")) == "N/A"
    ]
    ...
    result["coverage"] = {
        ...
        "duration_s": duration,
        "mlx_last_end_ratio": round(max(ends_mine) / duration, 4)
        if ends_mine and duration
        else None,
        "note": (
            "A last end far below the duration means the decode stopped early; that is "
...
def main() -> int:
    ...
    parser.add_argument(
        "--recorded", required=True, type=Path, help="recorded torch artifact from run_vibevoice.py"
    )
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument(
        "--max-tokens", type=int, default=None, help="defaults to the recorded run's max_new_tokens"
    )
    parser.add_argument(
        "--model-repo",
        default=MODEL_REPO,
        help="MLX VibeVoice repo; bf16 isolates implementation differences from quantization ones",
    )
    ...
    output = model.generate(
        audio, sampling_rate=sample_rate, max_tokens=max_tokens, temperature=0.0
    )
    ...
    document = {
        ...
        "question": (
            "Does mlx-audio's vibevoice_asr reproduce the recorded torch VibeVoice "
        ...
        "configuration": {
            ...
            "note": (
                "mlx-audio resamples to VibeVoice's 24 kHz internally; the "
                "fixture's own rate is declared rather than assumed."
            ),
        },
        "compared_against": {
            "artifact": str(args.recorded.relative_to(REPO))
            if args.recorded.is_absolute()
            else str(args.recorded),
            "runner": "model_tests/benchmark/run_vibevoice.py",
            "repo": TORCH_MODEL_REPO,
            "revision": TORCH_MODEL_REVISION,
            "configuration": recorded["configuration"],
            "timing": recorded["timing"],
            "memory": {key: value for key, value in recorded["memory"].items() if key != "samples"},
        ...
        "observed": {
            ...
            "note": (
                "MLX and PyTorch memory counters have different scopes and must not be "
