...
def main() -> None:
    ...
    crossing = [
        ...
        if any(
            ...
            for boundary in (args.period_seconds * index for index in range(1, args.repeats))
        )
    ]
    windows = []
        ...
        windows.append(
            {
                ...
                "normalized_segments_sha256": hashlib.sha256(serialized.encode()).hexdigest(),
                "text_sequence_sha256": hashlib.sha256(text_serialized.encode()).hexdigest(),
            }
        )
    first = windows[0]
    first_items = [
    ...
    for index in range(1, args.repeats):
        ...
        pairable = len(items) == len(first_items) and all(
            left["text"] == right["text"] and left["speaker"] == right["speaker"]
            for left, right in zip(first_items, items, strict=True)
        )
        drifts = (
            [
        ...
        paired_drift.append(
            {
                ...
                "median_absolute_timestamp_drift_s": statistics.median(drifts) if drifts else None,
                "p95_absolute_timestamp_drift_s": sorted(drifts)[math.ceil(0.95 * len(drifts)) - 1]
                if drifts
                else None,
                "within_timestamp_tolerance": bool(drifts)
                and max(drifts) <= (args.timestamp_tolerance_ms / 1000),
            }
        )
    result = {
        "schema_version": 2,
        ...
        "all_normalized_segments_equal": all(
            item["normalized_segments_sha256"] == first["normalized_segments_sha256"]
            for item in windows
        ),
        "all_text_sequences_equal": all(
            item["text_sequence_sha256"] == first["text_sequence_sha256"] for item in windows
        ),
        "all_text_speaker_sequences_and_timestamps_within_tolerance": all(
            item["pairable_text_and_speaker_sequence"] and item["within_timestamp_tolerance"]
            for item in paired_drift
        ),
    }
        ...
        json.dumps(
            {
                ...
                "maximum_timestamp_drift_s": max(
                    (
                        item["maximum_absolute_timestamp_drift_s"]
                        for item in paired_drift
