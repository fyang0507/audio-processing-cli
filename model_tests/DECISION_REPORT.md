...
| **FluidAudio 0.15.5 offline diarization CLI** | The tested *implementation* was FluidAudio's offline VBx pipeline, built from tag `v0.15.5` (`19600a485baa4998812e4654b70d2bab8f2c9949`) and provisioned with FluidInference's `speaker-diarization-coreml` model package. It emitted anonymous speaker intervals, including overlap-permitting output. | “FluidAudio” is the product/SDK and pipeline name here, not the name of a standalone diarization model. Keep overlap/short fragments in an abstention ledger; do not infer roles. |
| **Direct pyannote Community-1** | `pyannote.audio` 4.0.7 directly loaded `pyannote/speaker-diarization-community-1` revision `3533c8cf8e369892e6b79ff1bf80f7b0286a54ee`. On MPS it produced regular anonymous intervals without FluidAudio. | Offboarding is technically feasible, but it is not the current default: it needs gated Hub access, PyTorch MPS, and a PCM-WAV input workaround for the macOS TorchCodec/FFmpeg-dylib failure. Its CantoMap change agreement is stronger, but it is slower, heavier, and slightly worse on the SpiCE participant-only diagnostic. |
...
| --- | --- | --- | --- |
| **Interview transcription—mixed mono or dedicated participant/interviewer channels** | Fast Mandarin/Chinese dialects/English transcript; stable absolute speech bounds; anonymous speakers or capture-derived roles; explicit overlap handling. Word timing is optional. | Preserve dedicated channels when available, validate dominance/bleed, and deduplicate cross-talk. Otherwise use **FluidAudio quality → sample-exact reconciliation with overlap abstention**. Both paths converge on persistent **Qwen3-ASR 1.7B 8-bit**, `batch=1`, clearing the MLX cache after every batch (which at `batch=1` is every turn); choose **0.6B** only for minimum latency/footprint. Reattach the external bounds and confirm roles. Treat FluidAudio as this one diarizer backend, not the platform for Silero or the rest of the stack. | Direct Community-1 is a viable future adapter implementation, but its present MPS cost and decoder workaround do not justify default offboarding. Qwen container bounds are not timestamps. The 1.7B/0.6B accuracy comparison is Cantonese-only; validate Mandarin, English, and each target variety separately. Channel dominance is not diarization. The measured role label is an oracle, not identity inference. |
...
| **Dialect/LID/native-word-time or constrained-runtime exception** | Chinese dialect emphasis, dominant region LID, native word times, or lower memory/latency than VibeVoice. | Run the **full FireRedASR2S pipeline**. Keep LID off unless its region label is required; use ASR and punctuation batch size 4. Add external speaker intervals only when needed. | FireRed has no speaker output, and its regional LID cannot locate code switches. Native times are not yet validated as edit-grade. |
...
Keep FireRed for the explicit exception row: dialect/LID needs, native word times, or performance, memory, and latency constraints.
