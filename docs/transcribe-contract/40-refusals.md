...
## Traceable unavailable alignment

New Qwen and VibeVoice `alignment_unavailable` ledger entries retain their attempted interval plus `alignment: {"unit_id": "turn_25", "segment_ids": ["seg_27"], "code": "out_of_unit_bounds", "word_index": 0}` when that is the observed rejection. `word_index` is optional; the `alignment` object is absent in historical results. Measured boundary rejections additionally carry `alignment.boundary` as described below. The segment link identifies retained recognized text, not invented timing. [Alignment diagnostics](../alignment-diagnostics.md) defines the closed codes, retained structured stage files, default serialization allowance and explicit boundary-correction limit.

`timing_required_for_timestamps` retains its existing payload fields. Its `fix` distinguishes a result that never requested word timing from one whose `word_timestamps` outcome is `abstained`: without measured boundary evidence, the latter recommends removing `--timestamps` to preserve text and inspecting the segment's alignment abstention and retained structured response, explicitly without promising the same request will repair timing. An existing word-timing outcome with no usable timing also receives a preservation remedy rather than a request to enable it again. These remedies do not assert that word-timing delivery was repaired.

When canonical abstentions supply alignment associations, timing refusals also expose `alignment_rejections` as the unchanged saved entries: readable timestamps select entries linked to the affected segment; SRT/VTT include all alignment entries from the input responsible for the refusal. The field stays absent when no such evidence is supplied. An `out_of_unit_bounds` entry with `alignment.boundary` makes the remedy point to its original/unit bounds, measured overruns and selected cap. The agent may choose an explicit `--alignment-max-overrun-ms` or another supported stack on the original source, preserving scope and required capabilities and writing a new canonical output. The refusal neither selects a higher value automatically nor guarantees usable timing. Older evidence keeps its generic remedy. Any runnable subtitle rerun remedy preserves the saved aligner's configured limit alongside the source, requested capabilities, language and range.
