...
Aggregate only the required delivery rows: a failed row makes delivery FAIL; otherwise an unavailable required check makes it BLOCKED; otherwise all required rows must pass. Do not produce a passing verdict from an empty required-check list. Optional LRA/residual-noise misses do not enter that verdict unless attainment was required before the run. Keep those measurements visible without creating an automatic fine-tuning backlog.
