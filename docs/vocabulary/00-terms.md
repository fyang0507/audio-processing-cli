...
## Enhancement measurement scopes

`measurement_scope_comparison` compares recorded time intervals in two compatible saved enhancement or inspection reports. Its `basis` is `recorded_time_intervals`; `speech_reference_intervals` compares the union of nonempty, measured speech-reference spans, while `non_speech_region_intervals` compares individual non-speech intervals including multiplicity. Each available finding is `same` or `different`; missing scope evidence stays absent. These are time-scope findings, not equality of sample masks, metric values, processing phases or audio quality. Fixed original-interval verification and fresh detection retain separate measurements and outcome counts. See [report comparison](../report-comparison.md) for the exact evidence requirements and navigation projection.
