...
`provenance` carries three things and embeds a fourth. `stack` names what ran, `outcomes` says what became of each requested capability, and `observed` records supplied runtime measurements and conditional evidence. The fourth is `plan`: the executed plan verbatim, which these printouts omit because §1.2 already shows it in full — a result does not restate what the plan said, it appends what only running could tell you. The key-set test therefore compares against a real run, not against these trimmed prints.

This default-policy example has no `provenance.observed.alignment_corrections`. A run that explicitly allows larger endpoint overruns records each surviving clip there with `unit_id`, `segment_id`, `word_id`, `word_index`, `original_bounds`, `applied_bounds`, `unit_bounds`, both endpoint overruns in milliseconds and the selected `max_overrun_ms`. Rejected units instead retain `alignment_unavailable` abstentions and omit their word streams. With `--receipt`, stdout copies diagnostic abstentions into conditional `alignment_rejections` and the observed correction ledger into conditional `alignment_corrections`; the saved canonical result is unchanged by that presentation option. Completion and positive word counts do not establish that every requested word stream was delivered.
