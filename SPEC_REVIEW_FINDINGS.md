...
| O11 | `determinism_basis: "greedy decode"` was unsourced. | **Sourced twice.** `model_tests/benchmark/turn_attributed_mlx_asr/inference.py:133-136` passes `sampler=make_sampler(temp=0.0)`, and the capability record has `configuration/temperature = 0.0`. VibeVoice's text decode is likewise `do_sample=False` (`model_tests/benchmark/run_vibevoice.py:285-293`), which is now cited alongside its seeded-repeat hash. VOCABULARY now requires a `determinism_basis` to cite a repeat-hash measurement or a decode configuration; "by construction" alone is not a basis. |
