...
def publish_output(
    ...
    *,
    ...
    with temporary_output_path(output) as encoded_temp:
        with progress_stage(progress, "encode"):
            encode_output(
                source,
                final_wav,
                encoded_temp,
        ...
        with progress_stage(progress, "encoded-output-inspection"):
            ...
            if source_stage_report["status"] == "applied":
                final_region_evaluations = source_region_evaluations(
                    profile, staged, after_regional
                )
                unbounded_failures = [
                    str(item["region_id"])
        ...
        with progress_stage(progress, "publication"):
            os.replace(encoded_temp, output)

    with progress_stage(progress, "published-output-inspection"):
