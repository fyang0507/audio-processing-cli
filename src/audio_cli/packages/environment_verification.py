    ...
    *,
    ...
    for identifier, package_entry in document["packages"].items():
        ...
        if package is not None and package_entry.get("state") == "ready":
            ready_packages_by_environment.setdefault(package.environment, []).append(identifier)
    ...
    for name, environment in environments().items():
                ...
                failed.append(
                    {
                        ...
                        "packages": dependents,
                        "fix": ("audio packages pull --repair " + " ".join(dependents)),
                    }
                )
            continue
        ...
        blocked_by = [tool for tool in environment.requires_tool if toolchain.which(tool) is None]
        if blocked_by:
            ...
            failed.append(
                {
                    ...
                    "examples": {
                        n: {"locked": drift[n][0], "installed": drift[n][1]}
