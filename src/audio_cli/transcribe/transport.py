...
from .transport_types import ProcessRunner, StageFailure, StageOutcome
...
class StageTransport:
    ...
    def _json_stage(
        ...
        *,
        ...
        if not result_path.is_file():
            ...
            raise StageFailure(
                role, backend, detail[-4000:], outcome=transport_outcome
            )
        try:
            payload = json.loads(
        ...
        if not isinstance(payload, dict):
            raise StageFailure(
                role, backend, "stage result must be a JSON object",
                outcome=transport_outcome,
            )
        metrics = payload.get("metrics", {})
        if not isinstance(metrics, Mapping):
            raise StageFailure(
                role, backend, "stage result metrics must be a JSON object",
                outcome=transport_outcome,
            )
            ...
            if stage_walls is not None:
                ...
                for stage, value in stage_walls.items():
                    if isinstance(value, bool) or not isinstance(value, (int, float)):
                        raise TypeError(
                            "stage_wall_seconds values must be JSON numbers"
                        )
                    parsed = float(value)
        ...
        outcome = StageOutcome(
            ...
            ) if metric_peak_rss is not None or sampled_peak is not None else None,
            metric_peak_mps,
            internal_walls,
        )
        if completed.returncode not in {0, 4}:
            ...
            ) or completed.stderr.strip() or f"stage exited {completed.returncode}"
            raise StageFailure(
                role, backend, str(detail)[-4000:], outcome=outcome
            )
        self._notice(
    ...
    def diarize(
        ...
        *,
        ...
        command = [
            str(self._swift_product(
                checkout, product, product_path, product_sha256
            )),
            "process",
            str(audio),
            "--mode",
