...
def _manifest_role_source(package_id: str, role: str) -> dict[str, str]:
    ...
    if source.get("type") == "huggingface_multi":
        matches = [
            repository for repository in source.get("repos", ()) if repository.get("role") == role
        ]
        if len(matches) != 1:
...
def _resolved_roles(request: ResolvedRequest) -> dict[str, dict[str, Any]]:
    ...
    for role in _STAGE_ORDER:
        if role == "decode" or role not in backend_by_role:
            continue
        roles[role] = _role_template(role, backend_by_role[role], request, selected_by.get(role))
    return roles
...
def _capability_results(request: ResolvedRequest) -> dict[str, dict[str, Any]]:
    ...
    for capability in request.wants:
        ...
        entry: dict[str, Any] = {"satisfaction": "derived" if resolution == "add_on" else "native"}
...
def _ordered_packages(
    ...
    role_backends = {role: value["backend"] for role, value in roles.items()}
    ...
    for capability in request.wants:
        ...
        if package_id in by_id and package_id not in wanted:
            wanted.append(package_id)
        if (
            package_id == "fluidaudio"
            and "speaker-diarization-coreml" in by_id
...
def build_plan(
    ...
    *,
    ...
    unready = [
        package
        for package, record in zip(selected_packages, package_records, strict=True)
