...
def _record_metrics(outcomes: Sequence[StageOutcome], result: NormalizedResult) -> dict[str, Any]:
    ...
    for item in outcomes:
        if item.wall_seconds_by_stage:
            ...
            if residual < -0.005:
                raise ValueError(f"{item.role} internal wall metrics exceed its process wall")
    ...
    mps = {
        ...
        for item in outcomes
        if item.peak_mps_live_bytes is not None
    }
    if mps:
...
def _coverage(
    unfinished: Sequence[Mapping[str, Any]],
    *,
    total_units: int,
    completed_units: int,
    scope_start: float,
    ...
    covered = [] if watermark <= scope_start else [[round(scope_start, 6), round(watermark, 6)]]
...
def _write_result_file(
    ...
    *,
    ...
    protected_identities = (protected_identity,) if protected_identity is not None else ()
...
def _publish_result(
    ...
    *,
    ...
    try:
        ...
        raise refusals.output_is_canonical_input(output or target, target) from exc
...
def _outcomes(
    ...
    *,
    ...
    outcomes = dict.fromkeys(wants, "produced")
    wordless_events = {"[Environmental Sounds]", "[Silence]", "[Human Sounds]"}
    if "word_timestamps" in outcomes and any(
        item.get("text") and item.get("text") not in wordless_events and "words" not in item
