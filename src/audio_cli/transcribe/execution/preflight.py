    ...
    *,
    ...
    for record in plan.packages:
        ...
        if record.get("auto_fetch"):
            ...
            if source.get("type") == "url":
                ...
                location = (
                    ...
                    if override
                    else paths.models_dir() / str(source["filename"])
                )
                location_issue = None
                if not override:
                    ...
                    if root_issue is not None:
                        ...
                        location_issue = f"managed artifact parent is a symlink: {location.parent}"
                ...
                unusable = (
                    ...
                    or (location.exists() and (not location.is_file() or digest_changed))
                )
                if unusable:
                    failures.append(
                        {
                            ...
                            "actual": location_issue or ("missing, not a file, or digest changed"),
                        }
                    )
            continue
        ...
        if source.get("type") in {"huggingface", "huggingface_multi"}:
            hub_issues = hub_materialization_issues(catalog[identifier], dict(materialized))
            if hub_issues:
                failures.append(
                    {
                        "package": identifier,
                        ...
                        "expected": (
                            "snapshot directories, manifest-filtered files, and recorded byte size"
                        ),
                        "actual": hub_issues,
                    }
                )
        ...
        if checkout_spec is not None:
            ...
            checkout, location_issue = managed_checkout_path(catalog[identifier], checkout_value)
