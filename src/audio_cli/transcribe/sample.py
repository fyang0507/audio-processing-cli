...
from .result import serialization as result_serialization
from .result.types import NormalizedResult
...
def build_sample_output(
    *,
    ...
    if "word_timestamps" in requested:
        segment["words"] = [{"word_id": "w_0", "text": None, "start": None, "end": None}]

    values: dict[str, Any] = {}
    if "diarization" in requested:
        values["turns"] = [{"turn_id": "turn_0", "speaker": None, "start": None, "end": None}]
    if "vad" in requested:
        values["vad_regions"] = [{"start": None, "end": None}]
    if "lid" in requested:
        values["lid_regions"] = [{"start": None, "end": None, "language": None, "confidence": None}]
    if "overlapped_speech" in requested:
        values["overlapped_speech"] = [{"overlap_id": "overlap_0", "start": None, "end": None}]
    ...
    return result_serialization.serialize_result(placeholder)
