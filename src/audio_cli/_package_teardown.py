...
from ._package_core import ProvisioningError
    ...
    try:
        ...
        with os.scandir(directory_descriptor) as entries:
            for entry in entries:
                try:
                    total += facade_dependency(
                        "_owned_tree_bytes_at", _owned_tree_bytes_at
                    )(
...
def _delete_managed(target: Path) -> int:
    ...
    try:
        with bound_directory(
            ...
            opened = True
            before = facade_dependency(
                "_owned_tree_bytes_at", _owned_tree_bytes_at
            )(parent_descriptor, absolute.name)
            facade_dependency("_delete_at", _delete_at)(
                parent_descriptor, absolute.name
            )
            try:
                os.stat(
